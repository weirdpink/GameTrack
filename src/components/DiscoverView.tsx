...
export const DiscoverView: React.FC = () => {
  const { 
    ...
    lastTrendingFetch: s.lastTrendingFetch,
  ...
  useEffect(() => {
    if (trendingGames.length === 0 || Date.now() - lastTrendingFetch > 300_000) fetchTrending();
  }, [fetchTrending, trendingGames.length, lastTrendingFetch]);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0]?.isIntersecting && hasMore && !loadingDiscover) {
          const trimmed = query.trim();
  ...
  return (
    <div className="space-y-10">
      ...
      {!query.trim() && (
        <div className="space-y-14">
          <TabbedCuratedSection
            ...
            topThisMonth={discoverLists?.topThisMonth}
            bestAllTime={discoverLists?.bestAllTime}
            newReleases={discoverLists?.newReleases}
            mostHyped={discoverLists?.mostHyped}
          />
        </div>
      )}
      ...
      {discoverError && !loadingDiscover ? (
      ...
      ) : loadingDiscover && filteredGamesToDisplay.length === 0 ? (
      ...
      ) : filteredGamesToDisplay.length === 0 ? (
        <div className="border border-brand-border border-dashed rounded-none p-12 text-center flex flex-col items-center justify-center">
          ...
          <p className="text-brand-muted text-xs mt-1 max-w-sm">
            {selectedGenre 
              ? `No games match the "${selectedGenre}" genre filter in the current results.`
        ...
        <div className="space-y-6">
          ...
          <div ref={observerRef} className="pt-10 pb-16 flex flex-col items-center justify-center gap-3 border-t border-brand-border/40 mt-12">
            {loadingDiscover ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-medium tracking-wide text-brand-muted">You've reached the end of the feed</span>
                <span className="h-px w-12 bg-brand-border" />
              </div>
            )}
          </div>
