...
Both scripts talk directly to IGDB and are safe to re-run at any time. They stop
before changing anything if `IGDB_CLIENT_ID` / `IGDB_CLIENT_SECRET` are missing
or rejected.

```bash
npm run reset-metadata       # re-match every row against IGDB
npm run fetch-igdb-posters   # refresh artwork only
```

`reset-metadata` rewrites the IGDB id, year, genres, synopsis and critic score
for every library and wishlist row, and fixes the poster: **Steam-owned rows use
the Steam CDN portrait cover, every other row uses its IGDB cover**. Hand-uploaded
posters are never touched, previous ids are backed up to `data/`, and any stale
`media.rawg.io` URL is removed. Titles that cannot be matched with confidence are
left without an IGDB id rather than being linked to a different game.
...
├── scripts/
│   ├── reset-metadata.ts         # Re-match library/wishlist against IGDB
│   ├── fetch-all-igdb-posters.ts # Poster-only artwork refresh
│   └── lib/igdb-match.ts         # Shared conservative title matcher
├── src/
